...
public:
	...
	int GetFileNum(int* file, int dep);//获取文件夹里的文件数
	bool CancelFile(int* file, int dep);//文件夹转文件

	bool Balance(int* file, int dep, int num, bool less);//平衡文件夹里的节点数，使之成为num的倍数（如果less为真优先采用减少格子的方法）
	int GetIcoNum(int* file, int dep);//获取文件（夹）图标序号
	int SetIcoNum(int* file, int dep);//设置文件（夹）图标序号
};